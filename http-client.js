...
class HttpClient extends EventEmitter {
  ...
  formatTelegramUrl(input) {
    ...
    if (
      cleanInput.startsWith("https://t.me/") ||
      cleanInput.startsWith("http://t.me/")
    ) {
      return cleanInput;
    }
  ...
  async curlSetopts(input, customHeaders = [], retryCount = 0) {
    ...
    try {
      ...
      if (retryCount > 0) {
        ...
        this.emitProgress(
          4,
          `در حال انتظار برای جلوگیری از ریت لیمیت (${delay}ms)...`,
          30,
        );
      ...
      if (originalUrl.includes("t.me")) {
        ...
        if (proxyMethod === "google") {
          ...
          this.emitProgress(
            5,
            "در حال آماده‌سازی پروکسی گوگل ترنسلیت (2)...",
            50,
          );
          const urlObj = new URL(originalUrl);
          const pathname = urlObj.pathname || "";
          ...
          this.emitProgress(
            5,
            "در حال آماده‌سازی پروکسی گوگل ترنسلیت (3)...",
            50,
          );
          const urlObj = new URL(originalUrl);
          const pathname = urlObj.pathname || "";
          ...
          this.emitProgress(
            5,
            "در حال آماده‌سازی پروکسی گوگل ترنسلیت (4)...",
            50,
          );
          const urlObj = new URL(originalUrl);
          const pathname = urlObj.pathname || "";
        ...
        if (response.status === 429) {
          if (retryCount < maxRetries) {
            ...
            return {
              success: false,
              error:
                "تعداد تلاش‌های مجدد تمام شد. لطفاً چند دقیقه دیگر دوباره تلاش کنید.",
